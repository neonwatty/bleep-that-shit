...
test("user can upload mp3 and get transcription with Tiny model", async ({
  page,
}) => {
  try {
    await page.goto("http://bleep-that-shit.localhost:3000/transcription-view");
    ...
    await screenshotOnFailure(page, "mp3-with-tiny-model-failure");
    throw e;
  }
});

test("user can upload mp4 and get transcription with Tiny model", async ({
  page,
}) => {
  try {
    await page.goto("http://bleep-that-shit.localhost:3000/transcription-view");
    ...
    await screenshotOnFailure(page, "mp4-with-tiny-model-failure");
    throw e;
  }
});
...
test("user can cancel transcription with Tiny model", async ({ page }) => {
  try {
    ...
    await screenshotOnFailure(page, "cancel-with-tiny-model-failure");
    throw e;
  }
});

test("transcription-view: shows warning and disables transcribe for mismatched file type with Tiny model", async ({
  page,
}) => {
  try {
    await page.goto("http://bleep-that-shit.localhost:3000/transcription-view");
    ...
    await screenshotOnFailure(
      page,
      "transcription-view-mismatch-warning-with-tiny-model-failure"
    );
    throw e;
  }
});
