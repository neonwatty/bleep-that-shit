...
self.onmessage = async (event: MessageEvent) => {
  ...
  const { type, fileBuffer, audioData, model, language } = event.data;
  ...
  try {
    if (type === 'extract') {
      ...
      if (audioDataBuffer instanceof Uint8Array) {
        ...
        (self as any).postMessage({ type: 'extracted', audioBuffer: buffer }, [buffer]);
      } else {
    ...
    if (type === 'transcribe') {
      ...
      if (!(audioData instanceof Float32Array) && !audioData.length) {
        throw new Error(
          `Audio data must be a Float32Array, received: ${audioData.constructor.name}`
        );
      }
