...
test.describe('Video Transcription Test', () => {
  test('should extract and transcribe video file', async ({ page }) => {
    ...
    page.on('console', msg => {
      ...
      if (msg.text().includes('Worker') || msg.text().includes('extract') || msg.text().includes('video')) {
    ...
    const progressText = page.locator('.text-gray-600').filter({ hasText: /Loading|Processing|Extracting|Decoding/i });
    
    // Check progress updates
    for (let i = 0; i < 60; i++) { // Check for 2 minutes
