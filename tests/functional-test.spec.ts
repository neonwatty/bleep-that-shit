...
test.describe('Functional Transcription and Bleep Tests', () => {
  test('should perform full transcription and bleep workflow', async ({ page }) => {
    ...
    await expect(async () => {
      ...
      console.log(`Checking... Result div: ${hasResult}, Transcript: ${hasTranscript}, Error: ${hasError}`);
    ...
    const bleepInput = page.locator('input[placeholder*="bleep"], input[placeholder*="word"]').first();
  ...
  test('should handle sampler page functionality', async ({ page }) => {
    ...
    await expect(page.locator('h2').filter({ hasText: 'Step 2: Configure Sample' })).toBeVisible({ timeout: 10000 });
