...
test.describe('Bleep Page Tests', () => {
  ...
  test('should have file upload dropzone', async ({ page }) => {
    const dropzone = page
      .locator('div')
      .filter({ hasText: 'Drag and drop your audio or video file here or click to browse' })
      .first();
  ...
  test('should have numbered workflow sections', async ({ page }) => {
    ...
    for (let i = 1; i <= 3; i++) {
      const numberBadge = page
        .locator(`.bg-blue-500, .bg-green-500, .bg-indigo-500`)
        .filter({ hasText: i.toString() });
  ...
  test('should handle worker loading', async ({ page }) => {
    ...
    const workerErrors = consoleMessages.filter(
      msg => msg.toLowerCase().includes('worker') || msg.toLowerCase().includes('failed')
    );
