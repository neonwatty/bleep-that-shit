...
test.describe('Comprehensive Media Transcription and Bleep Tests', () => {
    ...
    test(`should transcribe and bleep ${description}`, async ({ page }) => {
      ...
      if (!hasError) {
        ...
        if (await bleepInput.isVisible()) {
          ...
          const matchButton = page.locator('button').filter({ hasText: /Match|Apply|Bleep/i }).last();
  ...
  test('should transcribe and bleep with all matching modes', async ({ page }) => {
    ...
    for (const mode of matchModes) {
      ...
      const checkbox = page.locator(`input[type="checkbox"]`).filter({ hasText: new RegExp(mode, 'i') });
    ...
    if (await bleepInput.isVisible()) {
      ...
      // Apply matching
      const matchButton = page.locator('button').filter({ hasText: /Match|Apply/i }).last();
      if (await matchButton.isVisible()) {
        await matchButton.click();
  ...
  test('should handle concurrent file uploads', async ({ page }) => {
    ...
    await expect(page.locator('text=/File loaded.*test_full\.mp3/i')).toBeVisible({ timeout: 5000 });
