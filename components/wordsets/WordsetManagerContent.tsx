...
export function WordsetManagerContent({ onWordsetUpdated }: WordsetManagerContentProps) {
  const {
    wordsets,
    searchQuery,
    setSearchQuery,
  ...
  return (
    <div>
      ...
      {currentView === 'list' && (
        <div className="space-y-6">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <button onClick={() => setCurrentView('import-export')} className="btn btn-secondary">
              📥 Import CSV
            </button>
            <button
              onClick={async () => {
      ...
      {currentView === 'import-export' && (
        <WordsetImportExport onImport={importCSV} onExportAll={exportCSV} onClose={handleCancel} />
      )}
      ...
      {currentView === 'delete-confirm' && deletingWordset && (
        <div className="space-y-4">
          <div className="rounded border border-orange-200 bg-orange-50 p-4">
            ...
            <p className="mt-2 font-semibold text-orange-900">⚠️ This action cannot be undone.</p>
          </div>
